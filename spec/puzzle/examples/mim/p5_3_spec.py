...
from puzzle.examples.mim import p5_3
from puzzle.problems import logic_problem
from puzzle.puzzlepedia import prod_config
from spec.mamba import *

with _description('p5_3'):
  with before.all:
    warehouse.save()
    prod_config.init()
    self.puzzle = p5_3.get()
  ...
  with description('solution'):
    ...
    with _it('parses puzzle'):
      node = logic_problem._parse(p5_3.SOURCE.split('\n'))
      print(astor.to_source(node))

    with _it('models puzzle'):
      model = logic_problem._model(p5_3.SOURCE.split('\n'))
      print(str(model))

    with it('exports a solution'):
      problem = self.puzzle.problems()[0]
      expect(problem.solution).to(look_like(p5_3.SOLUTION))
