 ...
 *
 * This approach is significantly slower than trial division.
 *
 * primesSieve(2000000) is substantially slower than calling the function in
...
func primesSieve(ceil int) {
	...
	for i := largestPrime + 2; len(knownPrimes) <= ceil; i += 2 {
		for i > min.Pos {
			// Remove this composite, increase the multiple, reinsert.
			heap.Pop(compositeHeap)
			min.Pos += min.Prime
			heap.Push(compositeHeap, min)
