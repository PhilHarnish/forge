...
type node2Link struct {
	Prefix string
	Node   *Node2
}

func NewNode2(matchWeight ...weight.Weight) *Node2 {
	result := Node2{}
...
func (node *Node2) Match(weight weight.Weight) {
...
func (node *Node2) Weight(weight weight.Weight) {
	node.MaxWeight = math.Max(node.MaxWeight, weight)
}

func (node *Node2) Link(path string, child *Node2) error {
	if len(path) == 0 {
