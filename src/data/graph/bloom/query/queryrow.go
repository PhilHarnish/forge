...
func NewQueryRowFromSources(query *Query, cells []weight.WeightedString) *queryRowFromSources {
...
func (row *queryRowFromSources) ReplaceSource(source int, cells []weight.WeightedString) {
	header := row.query.getColumnHeader()
	columnStart := header.offsets[source]
	columnEnd := header.offsets[source+1]
	if columnStart+len(cells) != columnEnd {
		panic(fmt.Sprintf("Column %d is %d cells wide: [%d, %d); given %d items",
			source, columnEnd-columnStart, columnStart, columnEnd, len(cells)))
	}
